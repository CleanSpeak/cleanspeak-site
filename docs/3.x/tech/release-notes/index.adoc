...
=== New
* Filter approval webhook (see link:/docs/3.x/tech/tutorials/handling-notifications#filter-approvals[Handling Notifications] for details)
** Added a new webhook that is fired by any time someone changes approves changes to the filter list.
** Resolves https://github.com/CleanSpeak/cleanspeak-issues/issues/12[Github Issue #12]
* Webhooks
** New description field to annotate your webhooks with information about them
** New toggles to enable/disable a webhook for specific event types. All event types are enabled by default to maintain previous behavior.
** Added new role to manage webhooks explicitly. Previously only admins could manage webhooks.

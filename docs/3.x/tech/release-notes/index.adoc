...
== Future Version (Tech Preview)
_Pending Release_

=== New
* Artificial intelligence assisted false positive detection

== Version 3.24.1
_Pending Release_

== Fixed
* Fixed setenv.sh so that commented lines are properly ignored in the cleanspeak.properties file
* Missing message in moderation could cause a page not to render properly in some cases

== Version 3.24.0
_Released Monday, July 15th, 2019_

:warning_message: This migration erases any existing external authentication configuration. It is highly recommended to make a database backup in case you need to roll back.
...
=== New
* OpenID Connect authentication method - Allows for easy login to the majority of external identity providers.

=== Removed
* Google Authentication - Use OpenID Connect instead
* SAML2 - Use OpenID Connect instead OR use OpenID Connect to link:https://fusionauth.io[FusionAuth] which provides SAML
* Passport - Use OpenID Connect instead (and consider upgrading to link:https://fusionauth.io[FusionAuth])
...
provided should cover most cases, but you may optionally modify or augment the unicode match ranges. Contact us at support@inversoft.com if you need assistance.
...
* In some cases Reverting a Blacklist Phrase in the Filter Approvals would cause an exception and would not be be reverted. This issue has been corrected. If you encounter this issue prior to version `3.22.0` contact
 support@inversoft.com for assistance.
...
* Feature. Added link:https://sightengine.com/[Sightengine] as an image filter provider.
* Enhancement. Add back the variations column in the Blacklist filter listing in the UI.

== Version 3.20.2
_Released Monday August 20th, 2018_

* Fixed. When a complex input string containing many numbers is filtered, if your filter configuration has one to many distinguishable
numbers an increased CPU load may occur which could cause performance degradation.

== Version 3.20.1
_Released Friday August 17th, 2018_

* Fixed. A CSS issue caused maintenance mode and the Filter test interface in the CleanSpeak webservice UI to render poorly. This
...
include::docs/3.x/tech/release-notes/__database-migration-warning.adoc[]
:elastic_message: A major version upgrade of Elastic Search requires a full rebuild of the search index. This will be done automatically on startup, this may cause additional load on the system until completed. Content and user search will be incomplete until the process is complete.
...
* Change. The `contentType` parameter on the link:../apis/content#filter-content[Filter Content] API now defaults to `text` instead of detecting type. This may enhance performance when filtering large content.
...
* Feature. Full support of regular expressions in Blacklist phrases. This feature supercedes the phrase feature introduced in version `3.15.0`. For details see <<../tutorials/filtering-phrases.adoc#,Blacklist Phrases>>.
...
* Fixed. Filter changes were not reloaded by the webservice after using the link:../apis/system#restore[Restore] API.
...
:warning_message: CleanSpeak has received a major UI overhaul and you may pleasantly surprised when you login after the upgrade. Contact support@inversoft.com if you have further questions or would like to schedule a demo of the new UI.
include::docs/3.x/tech/release-notes/__general-migration-warning.adoc[]
...
* Feature. Video Filtering added using Amazon Rekognition. See link:../tutorials/filtering-videos[how to setup video] filtering for details.
...
* Feature. <<../tutorials/filtering-phrases.adoc#,Blacklist Phrases>>
* Fixed. Minor bugs
* Internal. Upgraded prime.js
...
* Changed. API JSON response bodies now return empty objects. For example, the link:../apis/content#filter-content[Filter API] response will return an empty `matches` array even when no matches are returned.
...
* Fixed. `MaximumMatchLength` on Email and URL filter wasn't being honored when provided on the link:../apis/content#filter-content[Filter API].
...
* Fixed. Sending an integer or other non UUID string representation for the `contentItems[x].id` field on the link:../apis/content#batch-moderate-content[Batch Moderate] API returned `500` instead of a `400` with an error message.
* Fixed. Support sending an integer for the `contentItems[x].id` field on the link:../apis/content#batch-moderate-content[Batch Moderate] API.
* Fixed. Omitting the `callbackHeaders` caused the call to the `callbackURL` to fail on the link:../apis/content#batch-moderate-content[Batch Moderate] API.
* Fixed. Creating a Whitelist allowed entry without a locale was permitted on the Management Interface which will break the filter reload. This issue has been resolved and entries in this state will be corrected during upgrade. This issue was introduced in link:#version-3-11-0[Version 3.11.0]
...
* Feature. Added link:../apis/whitelist#search-for-allowed-words[Search for Allowed Words] API
* Feature. Added link:../apis/blacklist#bulk-delete-dictionary-entries-by-locale[Bulk Delete Dictionary Entries by Locale] API
* Feature. Added link:../apis/blacklist#bulk-delete-filter-entries-by-locale[Bulk Delete Filter Entries by Locale] API

== Version 3.10.0
_Released Wednesday June 7th, 2017_

* Feature. Added link:../apis/whitelist#retrieve-a-disallowed-phrase[Retrieve a Disallowed Phrase] API
* Feature. Added link:../apis/whitelist#retrieve-all-disallowed-phrases[Retrieve all Disallowed Phrases] API
* Feature. Added link:../apis/whitelist#delete-a-disallowed-phrase[Delete a Disallowed Phrase] API
* Feature. Added link:../apis/whitelist#create-a-disallowed-phrase[Create a Disallowed Phrase] API
* Feature. Added link:../apis/whitelist#retrieve-an-advanced-disallowed-phrase[Retrieve an Advanced Disallowed Phrase] API
* Feature. Added link:../apis/whitelist#retrieve-all-advanced-disallowed-phrases[Retrieve all Advanced Disallowed Phrases] API
* Feature. Added link:../apis/whitelist#delete-an-advanced-disallowed-phrase[Delete an Advanced Disallowed Phrase] API
* Feature. Added link:../apis/whitelist#create-an-advanced-disallowed-phrase[Create an Advanced Disallowed Phrase] API

* Fixed. Added missing APIs to the API key endpoint configuration.
...
* Feature. Added link:../apis/blacklist#retrieve-a-filter-entry[Retrieve a Filter Entry] API
* Feature. Added link:../apis/blacklist#search-for-filter-entries[Search for Filter Entries] API
* Feature. Added link:../apis/blacklist#create-a-filter-entry[Create a Filter Entry] API
* Feature. Added link:../apis/blacklist#delete-a-filter-entry[Delete a Filter Entry] API
* Feature. Added link:../apis/blacklist#update-a-filter-entry[Update a Filter Entry] API
* Feature. Added link:../apis/whitelist#retrieve-an-allowed-word[Retrieve an Allowed Word] API
* Feature. Added link:../apis/whitelist#retrieve-all-allowed-words[Retrieve all Allowed Words] API
* Feature. Added link:../apis/whitelist#delete-an-allowed-word[Delete an Allowed Word] API
* Feature. Added link:../apis/whitelist#create-an-allowed-word[Create an Allowed Word] API
* Feature. Added link:../apis/whitelist#update-an-allowed-word[Update an Allowed Word] API
* Feature. Added link:../apis/whitelist#retrieve-a-tag[Retrieve a Tag] API
* Feature. Added link:../apis/whitelist#retrieve-all-tags[Retrieve All Tags] API
* Feature. Added link:../apis/whitelist#delete-a-tag[Delete a Tag] API
* Feature. Added link:../apis/whitelist#create-a-tag[Create a Tag] API
* Feature. Added link:../apis/whitelist#update-a-tag[Update a Tag] API
* Feature. Optionally return filter matches on the link:../apis/content#moderate-content[Moderate Content] API response.
...
* Feature. Added link:../apis/blacklist#retrieve-a-dictionary-entry[Retrieve a Dictionary Entry] API
* Feature. Added link:../apis/blacklist#search-for-dictionary-entries[Search for Dictionary Entries] API
* Feature. Added link:../apis/blacklist#create-a-dictionary-entry[Create a Dictionary Entry] API
* Feature. Added link:../apis/blacklist#delete-a-dictionary-entry[Delete a Dictionary Entry] API
* Feature. Added link:../apis/blacklist#retrieve-a-tag[Retrieve a Tag] API
* Feature. Added link:../apis/blacklist#retrieve-all-tags[Retrieve all Tags] API
* Feature. Added link:../apis/blacklist#create-a-tag[Create a Tag] API
* Feature. Added link:../apis/blacklist#delete-a-tag[Delete a Tag] API
...
* API. Deprecate `checkOutUser` field in the link:../apis/users#retrieve-a-user[Retrieve a User] API and add `checkOutUserId`.

...
[role=release-note]
== Version 4.0.0
_Released Tuesday July 9th, 2024_

include::docs/3.x/tech/release-notes/__database-migration-warning-4.x.adoc[]

[WARNING.general-warning]
====
*Please Read*

CleanSpeak 4.0 changes the services/applications used. For more detailed information, see link:../upgrades-patches/3.x-4.x[3.x to 4.x]
====
...
** Previously, you would deploy a separate Management Interface and WebService. Now it's a single application. By default, the new CleanSpeak app will listen on port 8001.
...
=== Removed
* `cleanspeak-search-engine` is no longer distributed as a package. The product still uses ElasticSearch or OpenSearch as before, but **you must provide and maintain** a running search instance yourself. TODO - Reference clean install here.
...
** Daniel - we have a lot of dependency changes with this release but it seems like past release notes only mentioned major deps.

...
=== New
* Filter storage limiter
** For some applications, storing content was impossible due to the volume of content that was being filtered. This feature allows you to choose to keep some percent of the content so that storage is possible.
** Resolves https://github.com/CleanSpeak/cleanspeak-issues/issues/7

=== Enhanced
* Enhanced the URL filter to detect more cases where the user is avoiding the filter
** Explicitly fixes `exampledotcom` but also fixes many other embedded cases where `dot` is used instead of punctuation.
** Resolves https://github.com/CleanSpeak/cleanspeak-issues/issues/5
* Improved the OpenID Connect workflow so that we catch more problems with the configuration
** If the token url succeeds but the `access_token` field is missing then we will display a more specific error suggesting the problem is with the token endpoint instead of a generic something went wrong error.
** If the userinfo endpoint succeeds but does not have the `email` field then we will display a specific error suggesting that the email field was missing and something is wrong with the configuration.

=== Fixed
* Fixed the TLD API to use name in the parameters/URI instead of using the one in the body on `PUT`. The API will still accept calls if it is only in the body for compatibility but this is deprecated.
...
** Resolves https://github.com/CleanSpeak/cleanspeak-issues/issues/3
* Top level domain UI and link:/docs/3.x/tech/apis/url#retrieve-a-tld[API].
** Resolves https://github.com/CleanSpeak/cleanspeak-issues/issues/4
...
** Resolves https://github.com/CleanSpeak/cleanspeak-issues/issues/6

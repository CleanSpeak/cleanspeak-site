...
=== Setup
The only requirement to hookup CleanSpeak is to direct prometheus to the CleanSpeak endpoints. We used the following configuration for a CleanSpeak instance running locally on the docker host. (You can reach dockers host ip via `host.docker.internal` on macOS and Windows docker, and by default management interface runs on `8011` and webservice runs on `8001`)

...
To install on Windows or higher, use the ZIP bundle. Extract the CleanSpeak WebService ZIP file anywhere on the file system. Remember where you extract the file. This location will be referred to as `CLEANSPEAK_HOME`. We suggest extracting this file to a directory such as `C:\cleanspeak` on Windows.
...
Before starting CleanSpeak WebService for the first time, you’ll need to add your license Id and verify your database connection in the link:../reference/configuration[Configuration Reference] section. The name of this file is `cleanspeak.properties`.
...
Find the default database JDBC URL, username, and password values, and verify this information is correct. The default JDBC URL is configured for MySQL. If you’re using PostgreSQL, you’ll need to update the URL. See the `database.URL` property documentation in link:../reference/configuration[Configuration] for more information.

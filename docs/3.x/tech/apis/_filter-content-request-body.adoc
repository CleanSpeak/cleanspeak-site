...
|This parameter enables the whitelist filter. This parameter is unique in that when enabled by setting this parameter to `true` it implicitly disables all other filters. See link:../reference/tech/apis/kids-chat[Whitelist Filter].
|===

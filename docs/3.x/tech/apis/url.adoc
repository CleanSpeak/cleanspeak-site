...
`URL Whitelist Tags` are used to associate `Whitelisted URL`s with applications that wish to limit their scope. When a `Whitelisted URL` has a tag it can only affect applications that also contain that same tag.
...
[NOTE]
====
Since both of the create `TLD API`s use the same `URL`, providing both `tld` and `tlds` in the request will prefer the create single `TLD` variant.
====

...
(See link:../tutorials/handling-notifications[Handling Webhook Events] for more details)
...
|A list of enabled event types for this webhook. If a value in this map is false, then it will not recieve events of that type. The possible values for `eventType` are as follows (each has a related section in link:../tutorials/handling-notifications[Handling Webhook Events]):

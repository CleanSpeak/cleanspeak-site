...
|This parameter disables the Machine Learning filter completely. When this parameter is set to `true` all other nested `ml` parameters will be ignored.

|[field]#ml.models# [type]#[List<String>]# [required]#Required#
|Which ML models to pass content through. Valid options are `affinity`, `compassion`, `curiosity`, `identityAttack`, `insult`, `nuance`, `personalStory`, `profanity`, `reasoning`, `respect`, `severeToxicity`, `toxicity`, `threat`.
